...
public class RegionsFetcher implements Callable<LeafPage> {
   private static final Log log = LogFactory.getLog(RegionsFetcher.class);   
   private LeafPageRequest leafPageReq;

   public RegionsFetcher(LeafPageRequest leafPageReq) {
      ...
      while (nodeWalker.hasNext()) {
         ...
         if ("img".equalsIgnoreCase(name)) {
            ...
            if (attrs != null) {
               ...
               if (srcNode != null) {
                  imageUrls.add(srcNode.getNodeValue());
               }
            }
         }
